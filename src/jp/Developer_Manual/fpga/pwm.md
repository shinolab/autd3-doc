...
$$\begin{align*}
R &= \left[2P - \frac{D}{2}\right] \%\, T\\
F &= \left[2P + \frac{D+1}{2}\right] \%\, T
\end{align*}$$

ここで, $T(=512)$は周期である.

> NOTE: $T=512$とし, 位相として$2P$を使用しているのは, 振幅パラメータ (パルス幅) を約$\SI{8}{bit}$で指定できるようにするためである. 
> パルス幅は$0$から$T/2$までしか意味がないので, $T=512$とすることで$D=0$から$D=256$の257段階で指定できるようにしている.
